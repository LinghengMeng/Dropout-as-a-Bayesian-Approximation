...
class VariationalDense:
    ...
    def __init__(self, n_in, n_out, model_prob=0.9, model_lam=1e-2, name="hidden"):
        ...
        M_initializer = tf.initializers.truncated_normal(mean=0.0, stddev=0.01)
        self.model_M = tf.get_variable("{}_M".format(name), initializer=M_initializer([n_in, n_out]))
...
# q_loss = (tf.reduce_sum((q-model_y)**2)) / n_samples
...
if True:
    ...
    for i in range(len(Y_post)):
        plt.plot(X_pred[:, 0], Y_post[i], "b-", alpha=alpha)
    # plt.plot(X[:, 0], y, "r.")
